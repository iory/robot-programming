...
parser = argparse.ArgumentParser(description="Generate a 3D model of an AprilTag with customizable dimensions.")

# Add arguments
parser.add_argument('--total-thickness', type=float, default=0.005, help='Total thickness of the tag in meters.')
parser.add_argument('--bottom-thickness', type=float, default=0.00, help='Thickness of the bottom layer of the tag in meters.')
parser.add_argument('--markersize', type=float, default=0.04, help='Size of the marker in meters.')
parser.add_argument('--tag-id', type=int, default=0, help='ID of the AprilTag to generate.')

# Parse arguments
args = parser.parse_args()

total_thickness = args.total_thickness
bottom_thickness = args.bottom_thickness
markersize = args.markersize
tag_id = args.tag_id

thickness = total_thickness - bottom_thickness
block_size = markersize / 10.0
gen = AprilTagGenerator()
img = gen.generate(tag_id)

# --- 変更点 1: ベースメッシュの条件付き生成 ---
# ベースメッシュを格納するリストを初期化
base_mesh_list = []
if bottom_thickness > 0.0:
    # bottom_thicknessが正の場合のみメッシュを生成
    mesh = trimesh.creation.box((markersize, markersize, bottom_thickness))
    matrix = np.eye(4)
    matrix[0, 3] = markersize / 2.0 - block_size
    matrix[1, 3] = markersize / 2.0 - block_size
    mesh.apply_transform(matrix)
    # ベースは黒で表示（_black.stl に含まれるため）
    mesh.visual.vertex_colors = np.array([0, 0, 0])
    base_mesh_list.append(mesh) # リストに追加
# ----------------------------------------------
...
for y in range(10):
    for x in range(10):
        tmp = trimesh.creation.box((block_size, block_size, thickness))
        if img[y, x] == 255:
            # 白いピクセル -> プレビュー用に白く着色
            tmp.visual.vertex_colors = np.array([255, 255, 255])
        elif img[y, x] == 0:
            # 黒いピクセル -> プレビュー用に黒く着色
            tmp.visual.vertex_colors = np.array([0, 0, 0])
        else:
            continue  # 255でも0でもないピクセルはスキップ
        matrix = np.eye(4)
        matrix[0, 3] = x * block_size - block_size / 2.0
        matrix[1, 3] = y * block_size - block_size / 2.0
        matrix[2, 3] = bottom_thickness / 2.0 + thickness / 2.0
        tmp.apply_transform(matrix)

        if img[y][x] == 255:
            black_meshes.append(tmp) # 'black_meshes' には白ピクセルが入る
        else:
            white_meshes.append(tmp) # 'white_meshes' には黒ピクセルが入る

# --- 変更点 2: ベースメッシュの条件付き結合 ---
# base_mesh_listには、bottom_thickness > 0 の時だけメッシュが入っている
white_mesh = trimesh.util.concatenate(white_meshes + base_mesh_list)
# ----------------------------------------------
black_mesh = trimesh.util.concatenate(black_meshes)

# 出力ファイル作成(LinuxベースOSでは/tmpで受かるがWindowsだとFileNotFoundになるので明示的に出力ファイルを作成する)
base_dir = os.path.dirname(os.path.abspath(__file__))
save_dir = os.path.join(base_dir, "output")
os.makedirs(save_dir, exist_ok=True)
file_path_white = os.path.join(save_dir, f"{tag_id}_white.stl")
file_path_black = os.path.join(save_dir, f"{tag_id}_black.stl")

# black_mesh（白ピクセル）が空でないかチェック
if black_mesh.is_empty:
    print("警告: 白いピクセルに相当するメッシュがありません。")
else:
    black_mesh.units = 'm'
    black_mesh = black_mesh.convert_units('mm')
    black_mesh.export(file_path_white.format(tag_id))

# white_mesh（黒ピクセル + ベース）が空でないかチェック
if white_mesh.is_empty:
     print("警告: 黒いピクセル（およびベース）に相当するメッシュがありません。")
else:
    white_mesh.units = 'm'
    white_mesh = white_mesh.convert_units('mm')
    white_mesh.export(file_path_black.format(tag_id))


# プレビュー用のメッシュ結合
preview_meshes = []
if not white_mesh.is_empty:
    preview_meshes.append(white_mesh)
if not black_mesh.is_empty:
    preview_meshes.append(black_mesh)

if preview_meshes:
    mesh = trimesh.util.concatenate(preview_meshes)